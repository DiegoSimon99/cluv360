...
export const Reported = () => {
    ...
    const listPosts = () => {
        ...
        apiClient.post(`admin/posts/denuncias?page=${currentPage}`, data)
            .then(response => {
                        ...
                        <tbody className={`table-border-bottom-0 ${loading ? 'position-relative' : ''}`}>
                            ...
                            {posts.map((item, index) => (
                                <tr key={item.id}>
                                    ...
                                    <td>
                                        {item.contenido && (
                                            <span className="cursor-pointer" onClick={() => handleAlertDescription(item.contenido)}>Ver Descripción</span>
                                        )}
                                    </td>
