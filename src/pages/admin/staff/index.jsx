...
export const Index = () => {
  ...
  const listStaff = async () => {
    try {
      ...
      const response = await apiClient.post(`admin/staffs?page=${currentPage}`, data);
      setStaff(response.data.data);
      setTotalPages(response.data.last_page);
      setPerPage(response.data.per_page);
      setCurrentPageTable(response.data.current_page);
    } catch (error) {
      showNotification(error.response?.data?.message || "Ocurrio un error al listar personales", "error");
    } finally {
      setLoading(false);
    }
  };
  ...
  const editStaff = (id) => {
    navigate(`/admin/staffs/edit/${id}`);
  };

  const deleteStaff = (id) => {
    confirmAlert({
      title: "Confirmar eliminación",
      message: "¿Estás seguro que deseas eliminar este personal?",
      buttons: [
        {
          label: "Sí, eliminar",
          onClick: async () => {
            try {
              showLoading();
              const response = await apiClient.delete(`/admin/staffs/${id}`);
              ...
              showNotification(error.response?.data?.message || "Error al eliminar personal", "error");
            } finally {
              hideLoading();
            }
          },
      ...
      <div className="card">
        <div className="card-header align-items-center row">
          <div className="col-12 col-md-10 pt-0 pt-md-2 mb-4">
            <h5 className="mb-0 text-md-start text-center">Personales</h5>
          </div>
          <div className="col-12 col-md-2 mb-4">
            <button type="button" className="btn btn-primary w-100" onClick={() => createStaff()}>
              {" "}
              Nuevo Personal
            </button>
          </div>
        ...
        <div className="table-responsive text-nowrap pt-2 ps-4 pe-4">
          <table className="table table-hover">
            ...
            <tbody className={`table-border-bottom-0 ${loading ? "position-relative" : ""}`}>
              <LoaderTable loading={loading} cantidad={staffs.length}></LoaderTable>
