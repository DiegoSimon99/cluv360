...
export const Edit = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const response = await apiClient.get(`/admin/manual_payment_methods/${id}`);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      setLoading(true);
      const response = await apiClient.post(`/admin/manual_payment_methods/${id}`, data);
      if (response.data.success) {
      ...
      showNotification(error.response?.data?.message || "Error al actualizar Método de pago manual", "error");
    } finally {
      setLoading(false);
    }
  };
