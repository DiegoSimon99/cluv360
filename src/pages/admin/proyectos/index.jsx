...
export const Index = () => {
    ...
    const listProyectos = () => {
        ...
        apiClient.post(`admin/proyectos?page=${currentPage}`, data)
            .then(response => {
    ...
    const createProyecto = (e) => {
        navigate('/admin/proyectos/create');
    }

    const editProyecto = (id) => {
        navigate(`/admin/proyectos/edit/${id}`);
    }
            ...
            buttons: [
                {
                    ...
                    onClick: async () => {
                        try {
                            showLoading();
                            const response = await apiClient.delete(`/admin/proyectos/${id}`);
                            ...
                            showNotification(error.response?.data?.message || "Error al eliminar proyecto", 'error');
                        } finally {
                            hideLoading();
                        }
                    }
            ...
            <div className="card">
                <div className="card-header align-items-center row">
                    <div className="col-12 col-md-10 pt-0 pt-md-2 mb-4">
                        <h5 className="mb-0 text-md-start text-center">Proyectos</h5>
                    </div>
                        ...
                        <tbody className={`table-border-bottom-0 ${loading ? 'position-relative' : ''}`}>
                            ...
                            {proyectos.map((item, index) => (
                                <tr key={item.id}>
                                    ...
                                    <td>{item.titulo}</td>
                                    <td>{item.lotes_vendidos}</td>
                                    <td>{item.lotes_total}</td>
                                    <td>{item.metros}</td>
                                    <td>
                                        <NumberFormatter value={item.precio}></NumberFormatter>
                                    </td>
                                    <td>{item.moneda}</td>
                                    <td>
                                        <div className="dropdown">
                                            ...
                                            <div className="dropdown-menu">
                                                ...
                                                <button
                                                    aria-label="dropdown action option"
                                                    className="dropdown-item"
                                                    onClick={() => deleteProyecto(item.id)}
                                                >
