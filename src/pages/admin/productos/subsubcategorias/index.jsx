...
export const Index = () => {
    ...
    const listSubSubCategorias = () => {
        setLoading(true);
        const data = {
            search: search,
            paginate: paginate
        }
        apiClient.post(`admin/subsubcategories?page=${currentPage}`, data)
            .then(response => {
                setSubSubCategorias(response.data.data);
    ...
    const createSubSubCategoria = (e) => {
        navigate('/admin/subsubcategories/create');
    }

    const editSubSubCategoria = (id) => {
        navigate(`/admin/subsubcategories/edit/${id}`);
    }

    const deleteSubSubCategoria = (id) => {
        confirmAlert({
            title: 'Confirmar eliminación',
            message: '¿Estás seguro de que deseas eliminar esta sub subcategoría?',
            buttons: [
                {
                    label: 'Sí, eliminar',
                    onClick: async () => {
                        try {
                            showLoading();
                            const response = await apiClient.delete(`/admin/subsubcategories/${id}`);
                            ...
                            showNotification(error.response?.data?.message || "Error al eliminar sub subcategoría", 'error');
                        } finally {
                            hideLoading();
                        }
                    }
            ...
            <div className="card">
                <div className="card-header align-items-center row">
                    <div className="col-12 col-md-10 pt-0 pt-md-2 mb-4">
                        <h5 className="mb-0 text-md-start text-center">Sub SubCategorías</h5>
                    </div>
                    <div className="col-12 col-md-2 mb-4">
                        <button
                            type="button"
                            className="btn btn-primary w-100"
                            onClick={() => createSubSubCategoria()}
                ...
                <div className="table-responsive text-nowrap pt-2 ps-4 pe-4">
                    <table className="table table-hover">
                        ...
                        <tbody className={`table-border-bottom-0 ${loading ? 'position-relative' : ''}`}>
                            <LoaderTable
                                loading={loading}
                                cantidad={subsubCategorias.length}
                            ></LoaderTable>
                            {subsubCategorias.map((item, index) => (
                                <tr key={item.id}>
                                    ...
                                    <td>
                                        {item.image ? (
                                            <img src={item.image} alt="not-found" height={50} />
                                        ) : '-'}
                                    </td>
                                    <td>
                                        {item.banner ? (
                                        ...
                                        <div className="dropdown">
                                            ...
                                            <div className="dropdown-menu">
                                                <button
                                                    ...
                                                    href="#"
                                                    onClick={() => editSubSubCategoria(item.id)}
                                                >
                                                ...
                                                <button
                                                    aria-label="dropdown action option"
                                                    className="dropdown-item"
                                                    onClick={() => deleteSubSubCategoria(item.id)}
                                                >
