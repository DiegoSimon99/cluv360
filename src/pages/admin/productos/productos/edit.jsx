...
export const Edit = () => {
      ...
      await apiClient
        ...
        .then((response) => {
          if (response.data.success) {
            ...
            setFormData({
              name: data.name || "",
              exclusivo_gm360: data.exclusivo_gm360 || 0,
            ...
            if (data.attributes.length > 0) {
              setAttributesValue(data.select_attributes);
            }
            ...
            if (data.name_brand) {
              // Cargar las opciones de marca con la búsqueda por ID
              loadOptionsBrands(data.brand_id).then((brands) => {
                const existingBrand = brands.find((brand) => brand.value === data.brand_id);

                if (!existingBrand) {
                  setIsLoadingBrand(true);
                  apiClient
                    .get(`/admin/products/brand/select?search=${data.name_brand}`)
                    .then((response) => {
                    ...
                    .catch((error) => {
                      showNotification("Error al obtener la marca específica:", error, "error");
                    })
                    .finally(() => {
        ...
        .catch((error) => {
          showNotification(error.response?.data?.message || "Error al consultar producto", "error");
        })
        .finally(() => {
          setIsLoading(false);
        });
  ...
  const handleSubmit = async (e) => {
    ...
    newFiles.forEach((file, index) => {
      if (file?.preview && file?.isNew) {
        // Asegúrate de que tenga las propiedades necesarias
    ...
    apiClient
      ...
      .catch((error) => {
        showNotification(error.response?.data?.message || "Error al actualizar el producto", "error");
      })
      .finally(() => {
        setLoading(false);
      });
  ...
  const generateVariations = (choice_options, colors, colorEnabled, existingVariations = []) => {
    // Si no hay atributos ni colores, no generamos variaciones
    if ((!choice_options || choice_options.length === 0) && colors.length === 0) return [];

    // Extraer valores de atributos si existen
    let attributeValues = [];
    ...
    if (attributeValues.length === 0 && colors.length === 0) return [];
    ...
    // **Generar variaciones basadas en colores**
    if (!colorEnabled && colors.length > 0) {
      ...
      // **Si no hay colores, usar solo atributos**
    ...
    // **Mantener los valores previos si existen en la base de datos**
    return newVariations.map((newVariation) => {
      const existingVariation = existingVariations.find((v) => v.combination === newVariation.combination);

      return {
        ...newVariation,
        price: existingVariation ? existingVariation.price : newVariation.price,
        sku: existingVariation ? existingVariation.sku : newVariation.sku,
        quantity: existingVariation ? existingVariation.quantity : 10,
      };
    });
  };

  // **useEffect para inicializar variaciones al editar y modificar datos**
  useEffect(() => {
    ...
    // Llamamos a la función pasando las variaciones existentes del backend
    const newVariations = generateVariations(
      formData.choice_options,
      formData.colors,
      colorEnabled,
      formData.variations || []
    );
                ...
                <div className="col-sm-3 d-flex align-items-center">
                  <div className="form-check form-switch">
                    <input
                      ...
                      type="checkbox"
                      name="colors_active"
                      checked={colorEnabled ? false : true}
                ...
                <div className="col-sm-7">
                  <Select
                    ...
                    onChange={(selectedAttributes) => {
                      setAttributesValue(selectedAttributes);
                      setFormData({
                        ...formData,
                        ...
                        choice_options: selectedAttributes.map((attr) => ({
                          attribute_id: parseInt(attr.value, 10),
                          values: [],
