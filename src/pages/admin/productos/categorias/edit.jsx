...
export const Edit = () => {
        ...
        apiClient.get(`/admin/categories/${id}`)
            .then(response => {
                if (response.data.success) {
                    ...
                    setPhotos({
                        photo1: data.banner ? { name: "image_uploaded_1", preview: data.banner, isNew: false } : null,
                        photo2: data.icon ? { name: "image_uploaded_2", preview: data.icon, isNew: false } : null
                    });
                } else {
            ...
            .catch((error) => {
                showNotification(error.response?.data?.message || "Error al obtener categoría", 'error');
            })
            .finally(() => {
                setIsLoading(false);
            });
    ...
    const handleSubmit = async (e) => {
        ...
        if (photos.photo1?.isNew) {
            data.append("banner", photos.photo1);
        }

        if (photos.photo2?.isNew) {
            data.append("icon", photos.photo2);
        }
        ...
        setLoading(true);
        apiClient.post(`/admin/categories/${id}`, data)
            .then(response => {
                if (response.data.success) {
            ...
            .catch((error) => {
                showNotification(error.response?.data?.message || "Error al actualizar categoría", 'error');
            })
            .finally(() => {
                setLoading(false);
            });
            ...
            <div className="card">
                <div className="card-header align-items-center row">
                    <div className="col-12 col-md-10 pt-0 pt-md-2 mb-4">
                        <h5 className="mb-0 text-md-start text-center">Información de categorías</h5>
                    </div>
                </div>
                <div className="card-body">
                    {!isLoading ? (
                        <form onSubmit={handleSubmit}>
                            ...
                            <div className="row mb-3">
                                <label className="col-sm-2 col-form-label">Tasa de comisión</label>
                                <div className="col-sm-10">
                                    <input
                                        type="text"
                                        className='form-control'
                                        name='commision_rate'
                                        value={formData.commision_rate}
                                        onChange={handleChange} />
                                </div>
                            </div>
