...
export const Index = () => {
    ...
    const listProductos = () => {
        ...
        apiClient.post(`admin/products/seller/lista?page=${currentPage}`, data)
            .then(response => {
    ...
    useEffect(() => {
        const fetchSellers = async () => {
            try {
                const response = await apiClient.get("/admin/products/seller/users");
                setSellers(response.data);
            } catch (error) {
                showNotification("Ocurrió un error al cargar los vendedores");
            }
        };
    ...
    const handleCheckboxChange = async (e, id) => {
        ...
        if (name === 'todays_deal') {
            setProductos((prevItems) =>
                prevItems.map((item) =>
                    item.id === id ? { ...item, todays_deal: newStatus } : item
                )
            );
            try {
        ...
        <>
            <div className="card">
                <div className="card-header align-items-center row">
                        ...
                        >
                            ...
                            {sellers.map((item) => (
                                <option key={item.id} value={item.user.id}>
                                    {item.user.shop.name+" ("+item.user.name+")"}
                                </option>
                            ))}
                        </select>
                    </div>
                    ...
                    <div className="col-0 col-md-2 mb-1"></div>
                    <div className="col-12 col-md-2 mb-1">
                        <br />
                        ...
                        <tbody className={`table-border-bottom-0 ${loading ? 'position-relative' : ''}`}>
                            ...
                            {productos.map((item, index) => (
                                <tr key={item.id}>
                                    ...
                                    <td>
                                        <img src={item.logo} alt="not-found" height={50} />
                                        {item.name}
                                    </td>
