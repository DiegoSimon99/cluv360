...
export const Edit = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const response = await apiClient.get(`/admin/pick_up_points/edit/${id}`);
        setFormData(response.data.data);
      } catch (error) {
        showNotification(error.response?.data?.message || "Error al consultar Punto de recogida", "error");
      }
    };
    fetchData();
  ...
  const handleSubmit = async (e) => {
    ...
    setLoading(true);
    try {
      const response = await apiClient.patch(`/admin/pick_up_points/${id}`, formData);
