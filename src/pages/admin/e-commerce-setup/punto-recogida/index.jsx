...
export const Index = () => {
  ...
  const listPickUpPoints = async () => {
    try {
      ...
      const response = await apiClient.post(`admin/pick_up_points?page=${currentPage}`, data);
  ...
  const createPickUpPoint = (e) => {
    navigate("/admin/pick_up_points/create");
  };

  const editPickUpPoint = (id) => {
    navigate(`/admin/pick_up_points/edit/${id}`);
  };

  const deletePickUpPoint = (id) => {
    confirmAlert({
      title: "Confirmar eliminación",
      message: "¿Estás seguro de que deseas eliminar este punto de recogida?",
      buttons: [
        {
          label: "Sí, eliminar",
          onClick: async () => {
            try {
              showLoading();
              const response = await apiClient.delete(`/admin/pick_up_points/${id}`);
              ...
              showNotification(error.response?.data?.message || "Error al eliminar punto de recogida", "error");
            } finally {
              hideLoading();
            }
          },
      ...
      <div className="card">
        <div className="card-header align-items-center row">
          <div className="col-12 col-md-10 pt-0 pt-md-2 mb-4">
            <h5 className="mb-0 text-md-start text-center">Punto de recogida</h5>
          </div>
          <div className="col-12 col-md-2 mb-4">
            <button type="button" className="btn btn-primary w-100" onClick={() => createPickUpPoint()}>
              {" "}
          ...
          <table className="table table-hover">
            <thead>
              <tr>
                ...
                <th>Manager</th>
                <th>Ubicación</th>
                <th>Contacto de la estación de recogida</th>
                <th>Estado</th>
                <th>Acciones</th>
              </tr>
            </thead>
            <tbody className={`table-border-bottom-0 ${loading ? "position-relative" : ""}`}>
              <LoaderTable loading={loading} cantidad={pickUpPoints.length}></LoaderTable>
              {pickUpPoints.map((item, index) => (
                <tr key={item.id}>
                  <td>{index + 1 + (currentPageTable - 1) * perPage}</td>
                  <td>{item.name}</td>
                  <td>{item.staff ? item.staff.user.name : "No Manager"}</td>
                  <td>{item.address}</td>
                  <td>{item.phone}</td>
                  <td>
                    {item.pick_up_status != 1 ? (
                      <div className="badge rounded-pill text-bg-danger">Close</div>
                    ) : (
                      <div className="badge rounded-pill text-bg-success">Open</div>
                    )}
                  </td>
                  <td>
                    <div className="dropdown">
                      ...
                      <div className="dropdown-menu">
                        <button
                          ...
                          href="#"
                          onClick={() => editPickUpPoint(item.id)}
                        >
                        ...
                        <button
                          aria-label="dropdown action option"
                          className="dropdown-item"
                          onClick={() => deletePickUpPoint(item.id)}
                        >
