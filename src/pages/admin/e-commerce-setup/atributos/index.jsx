...
export const Index = () => {
  ...
  const listAttribute = () => {
    ...
    apiClient
      .post(`admin/attributes?page=${currentPage}`, data)
      .then((response) => {
  ...
  const editAttribute = (id) => {
    navigate(`/admin/attributes/edit/${id}`);
  };

  const deleteAttribute = (id) => {
    confirmAlert({
      title: "Confirmar eliminación",
      message: "¿Estás seguro de que deseas eliminar este atributo?",
      buttons: [
        {
          label: "Sí, eliminar",
          onClick: async () => {
            try {
              showLoading();
              const response = await apiClient.delete(`/admin/attributes/${id}`);
              ...
              showNotification(error.response?.data?.message || "Error al eliminar atributo", "error");
            } finally {
              hideLoading();
            }
          },
      ...
      <div className="card">
        <div className="card-header align-items-center row">
          <div className="col-12 col-md-10 pt-0 pt-md-2 mb-4">
            <h5 className="mb-0 text-md-start text-center">Atributos</h5>
          </div>
          <div className="col-12 col-md-2 mb-4">
            <button type="button" className="btn btn-primary w-100" onClick={() => createAttribute()}>
              {" "}
              Nuevo Atributo
            </button>
          </div>
        ...
        <div className="table-responsive text-nowrap pt-2 ps-4 pe-4">
          <table className="table table-hover">
            ...
            <tbody className={`table-border-bottom-0 ${loading ? "position-relative" : ""}`}>
              <LoaderTable loading={loading} cantidad={attributes.length}></LoaderTable>
