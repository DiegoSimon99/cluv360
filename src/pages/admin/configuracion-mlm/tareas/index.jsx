...
export const Index = () => {
    ...
    const listTareas = () => {
        ...
        apiClient.post(`admin/config/tareas?page=${currentPage}`, data)
            .then(response => {
    ...
    const createTarea = (e) => {
        navigate('/admin/config/cron-job-settings/create');
    }

    const editTarea = (id) => {
        navigate(`/admin/config/cron-job-settings/edit/${id}`);
    }
            ...
            buttons: [
                {
                    ...
                    onClick: async () => {
                        try {
                            showLoading();
                            const response = await apiClient.delete(`/admin/config/tareas/${id}`);
                            ...
                            showNotification(error.response?.data?.message || "Error al eliminar tarea", 'error');
                        } finally {
                            hideLoading();
                        }
                    }
            ...
            <div className="card">
                <div className="card-header align-items-center row">
                    <div className="col-12 col-md-10 pt-0 pt-md-2 mb-4">
                        <h5 className="mb-0 text-md-start text-center">Configuración de Tareas</h5>
                    </div>
                    ...
                    <table className="table table-hover">
                        <thead>
                            <tr>
                                ...
                                <th>Usuario</th>
                                <th>Frecuencia</th>
                                <th>Fecha de creación/actualización</th>
                                <th>Acciones</th>
                            </tr>
                        </thead>
                        <tbody className={`table-border-bottom-0 ${loading ? 'position-relative' : ''}`}>
                            ...
                            {tareas.map((item, index) => (
                                <tr key={item.id}>
                                    ...
                                    <td>{item.message}</td>
                                    <td>{item.usuarios}</td>
                                    <td>{item.minute+" "+item.hour+" "+item.day+" "+item.month+" "+item.day_business}</td>
