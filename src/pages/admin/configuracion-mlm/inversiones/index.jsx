...
export const Index = () => {
  ...
  useEffect(() => {
    const fetchInversiones = async () => {
      try {
        const response = await apiClient.get("/admin/config/inversion");
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await apiClient.patch("/admin/config/inversion", data);
      if (response.data.success) {
  ...
  const agregarFila = () => {
    setInversiones([...inversiones, { amount: "", porcentaje: "" }]);
  };

  const eliminarFila = (index) => {
    const nuevos = [...inversiones];
    nuevos.splice(index, 1);
    setInversiones(nuevos);
  };
      ...
      <div className="card">
        <div className="card-header align-items-center row">
          <div className="col-12 col-md-10 pt-0 pt-md-2 mb-4">
            <h5 className="mb-0 text-md-start text-center">Configuración de Inversiones</h5>
          </div>
        </div>
