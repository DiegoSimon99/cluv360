...
export const Index = () => {
    ...
    const listRangos = () => {
        ...
        apiClient.post(`admin/config/rangos?page=${currentPage}`, data)
            .then(response => {
    ...
    const createRango = (e) => {
        navigate('/admin/config/rangos/create');
    }

    const editRango = (id) => {
        navigate(`/admin/config/rangos/edit/${id}`);
    }

    const deleteRango = (id) => {
        confirmAlert({
            title: 'Confirmar eliminación',
            message: '¿Estás seguro de que deseas eliminar este rango?',
            buttons: [
                {
                    label: 'Sí, eliminar',
                    onClick: async () => {
                        try {
                            showLoading();
                            const response = await apiClient.delete(`/admin/config/rangos/${id}`);
                            ...
                            showNotification(error.response?.data?.message || "Error al eliminar rango", 'error');
                        } finally {
                            hideLoading();
                        }
                    }
            ...
            <div className="card">
                <div className="card-header align-items-center row">
                    <div className="col-12 col-md-10 pt-0 pt-md-2 mb-4">
                        <h5 className="mb-0 text-md-start text-center">Rangos</h5>
                    </div>
                ...
                <div className="table-responsive text-nowrap pt-2 ps-4 pe-4">
                    <table className="table table-hover">
                        <thead>
                            <tr>
                                <th>Orden de rango</th>
                                <th>Nombre</th>
                                <th>Frase</th>
                                <th>Puntos personales</th>
                                <th>Puntos de calificación</th>
                                <th>Directos Activos</th>
                                <th>Cantidad de rangos activos</th>
                                <th>Monto Premio</th>
                                <th>Habilitar Niveles</th>
                                <th>Acciones</th>
                            </tr>
                        </thead>
                        <tbody className={`table-border-bottom-0 ${loading ? 'position-relative' : ''}`}>
                            ...
                            {rangos.map((item, index) => (
                                <tr key={item.id}>
                                    <td>{item.nivel_rango}</td>
                                    <td>{item.nombre}</td>
                                    <td>{item.descripcion}</td>
                                    <td>{item.pts_personales}</td>
                                    <td>{item.puntos}</td>
                                    <td>{item.directos}</td>
                                    <td>
                                        {item.cantidad_rango_id.map(value => (
                                            <p className="mb-0" key={item.id}>{`* ${value.cantidad} ${value.nombre}`}</p>
                                        ))}

                                    </td>
                                    <td>
                                        <NumberFormatter value={item.monto_premio}></NumberFormatter>
                                    </td>
                                    <td>{`${item.habilitar} nivel`}</td>
                                    <td>
