...
export const Edit = () => {
    ...
    useEffect(() => {
        setIsLoading(true);
        apiClient.get(`/admin/config/rangos/edit/${id}`)
            .then(response => {
                if (response.data.success) {
                    ...
                    setFormData({
                        nombre: data.nombre || "",
                        directos: data.directos || 0,
                        pts_personales: parseFloat(data.pts_personales) || 0,
                        puntos: parseFloat(data.puntos) || 0,
                        monto_premio: parseFloat(data.monto_premio) || 0,
                        habilitar: data.habilitar || 0,
                        nivel_rango: data.nivel_rango || 0,
                        descripcion: data.descripcion || "",
                    });
                    setRangos(data.cantidad_rango_id);
                    setBonos(data.bonos);
                    if (data.photo) {
                        setFile({
                            name: "image_uploaded",
            ...
            .catch((error) => {
                showNotification(error.response?.data?.message || "Error al obtener rango", 'error');
            })
            .finally(() => {
                setIsLoading(false);
            });
    ...
    const handleSubmitRango = async (e) => {
        ...
        setLoading(true);
        apiClient.post(`/admin/config/rangos/update/${id}`, data)
            .then(response => {
