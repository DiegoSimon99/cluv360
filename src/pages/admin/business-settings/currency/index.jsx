...
export const Index = () => {
  ...
  const listCurrency = async () => {
    ...
    try {
      const response = await apiClient.post(`admin/currency?page=${currentPage}`, data);
  ...
  const editCurrency = (id) => {
    navigate(`/admin/currency/edit/${id}`);
  };

  const handleCheckboxChange = async (e, id) => {
    const newStatus = e.target.checked ? 1 : 0;

    setCurrency((prevItems) => prevItems.map((item) => (item.id === id ? { ...item, status: newStatus } : item)));
    const data = {
      id: id,
      status: newStatus,
    };
    try {
      const response = await apiClient.post("/admin/currency/updateStatus", data);
      ...
      <div className="card">
        <div className="card-header align-items-center row">
          <div className="col-12 col-md-10 pt-0 pt-md-2 mb-4">
            <h5 className="mb-0 text-md-start text-center">Monedas</h5>
          </div>
          <div className="col-12 col-md-2 mb-4">
            <button type="button" className="btn btn-primary w-100" onClick={() => createCurrency()}>
              {" "}
              Nueva Moneda
            </button>
          </div>
          ...
          <table className="table table-hover">
            <thead>
              <tr>
                ...
                <th>Tipo de cambio (1 USD = ?)</th>
                <th>Estado</th>
                <th>Acciones</th>
              </tr>
            </thead>
            <tbody className={`table-border-bottom-0 ${loading ? "position-relative" : ""}`}>
              <LoaderTable loading={loading} cantidad={currencies.length}></LoaderTable>
